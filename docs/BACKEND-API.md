...
Вслед за `member-joined` бэкенд сам пишет в ленту системную запись — «„Гром“ 042 встал
на рейд» — и рассылает её обычным `message-added`. У неё стоит `kind: 'system'`, и текст
складывается один раз, при входе: смена позывного его не перепишет. То же и на `updateMember`:
если поменялись позывной, бортовой номер или силуэт, в ленту идёт запись о том, что было
и что стало («„Буран“ 517 теперь „Сирена“ 900, силуэт: тральщик → торпедный катер»).
Складывать такой текст может только бэкенд — он один знает, каким корабль был до правки.
Отдельного метода для системных записей нет: их пишет сам канал.
