...
запись и рассылает её обычным `message-added`. У таких записей стоит `kind: 'system'`, корабль
в них назван целиком в каждой половине фразы, а изменившееся помечено звёздочками — интерфейс
делает из пометки выделение. Кавычки в пометку не входят:
...
и текст остаётся прежним, даже если корабль потом переименуют ещё раз. Отдельного метода
для системных записей нет: их пишет сам канал.
