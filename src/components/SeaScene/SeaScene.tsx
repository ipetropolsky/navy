...
// Перспектива в долях высоты воды: где стоит ближний корабль и на сколько поднимается дальний.
// Доли, а не проценты сцены, потому что вода занимает разную её часть на телефоне и на десктопе,
// и общая мерка — единственный способ получить одинаковую сцену. Числа взяты из прежней
// раскладки, где отсчёт шёл от низа сцены: 8% сцены при воде в 44% это 0.182 высоты воды,
// а шаг 32% сцены — 0.727. Те же значения продублированы в стилях (@sea-near-edge,
// @sea-depth-span): по ним ставится берег острова.
const SEA_NEAR_EDGE = 0.182;
const SEA_DEPTH_SPAN = 0.727;
...
export default function SeaScene({ members, myId, morseFeeds, ready, onMoveShip }: SeaSceneProps) {
    ...
    const slotStyle = (member: Member, width: number): CSSProperties => {
        ...
        return {
            ...
            // у горизонта пустую полосу. Ближний край не прижат к самому низу — под ближним
            // кораблём нужна вода, иначе он «висит» на краю кадра.
            bottom: `calc((100% - var(--horizon)) * ${(SEA_NEAR_EDGE + (1 - depth) * SEA_DEPTH_SPAN).toFixed(4)})`,
