 ...
 * не должен. Телефон не в счёт: неба в свёрнутом кадре полоса в 60–135px, и тридцать пикселей
 * съели бы добрую его половину. Сдвиг там нулевой в обеих раскладках — разворот его не добавляет,
 * иначе звёзды переезжали бы на глазах. Месяц на телефоне и вовсе отмерен не от горизонта,
 * а от строки состояния канала — тоже в обеих раскладках одинаково.
 *
 ...
 * см. --sky-tile.
 */
test('небо и месяц опущены к воде, а на телефоне остаются на месте', async ({ page }) => {
    ...
    // высота неба, и месяц стоит на той же её части. Мерка эта десктопная: на телефоне месяц
    // отмерен от строки состояния, и в обеих раскладках одинаково (см. ниже).
    ...
    // А на телефоне месяц стоит на строке состояния — и разворот его с неё не сгоняет.
    // Отдельного правила у развёрнутого кадра тут нет и быть не должно: оно тут стояло,
    // и месяц на развороте съезжал вниз почти на семьдесят пикселей — хотя разворот открывает
    // небо, а не подтаскивает месяц к воде. От воды он теперь отъезжает вместе с горизонтом,
    // и это ровно то, что видно глазу: неба стало больше, месяц стал выше.
    expect(frames.phone.moonTop, 'месяц на телефоне сошёл со строки состояния').toBe(MOON_TOP_MOBILE);
    expect(frames.phoneFull.moonTop, 'разворот согнал месяц со строки состояния').toBe(MOON_TOP_MOBILE);
    expect(
        frames.phoneFull.moonAbove,
        'в развёрнутом кадре месяц не поднялся над водой выше, чем в свёрнутом'
    ).toBeGreaterThan(frames.phone.moonAbove);
});
