...
/**
 * То же самое для первой половины разворота в боковую раскладку, мс. Ждёт она дольше:
 * блоку мало уйти за кромку — приехать ему некуда, пока кадр не встал в свой новый размер
 * (см. switchLayout и .appReserve в стилях). Запас поэтому отсчитан от разворота
 * (@expand-seconds — 0.42s), а не от переезда.
 */
...
export default function App() {
    ...
    // Чего ждёт вторая половина переезда. Ждёт она двоих, и оба обязаны отметиться: сам блок —
    // что ушёл за кромку, и кадр — что доехал до нового размера. Второго ждут только в развороте
    // (см. switchLayout): в переезде по кнопке кадр не двигается, и ждать нечего.
    //
    // Отметки лежат в ref, а не в состоянии: перерисовывать по ним нечего — они лишь сводят
    // два события в одно.
    ...
    // кнопке.
    //
    // Место под панель кадр бронирует сразу, тем же нажатием (см. .appReserve в стилях):
    // раз уж известно, что разговор встанет сбоку, кадру незачем сперва раздаваться во всё
    // окно, а потом отдавать треть обратно. Прежде он так и делал — расхлопывался во всю
    // ширину и в тот же миг, когда панель занимала своё место, отскакивал назад на её ширину.
    // Теперь кадр идёт к своему размеру по прямой, а справа от него всю дорогу пустое место,
    // в которое разговор и приезжает.
    //
    // Бронь едет теми же секундами и той же кривой, что и ширина приложения: ширина кадра —
    // их разность, и разойдись они хоть чем, прямая перестала бы быть прямой. Отсюда и порядок
    // движений: разговор уходит вниз сразу, а приезжает не когда ушёл, а когда кадр доехал, —
    // до этой минуты бронь ещё уже панели, и приезд в неё был бы тем же скачком, только мельче.
    //
    ...
    const switchLayout = useCallback(() => {
        ...
        setHoldUnder(toSide);
        gone.current = false;
        // В развороте вторую половину переезда ждёт ещё и кадр; в переезде по кнопке кадр
        // не двигается вовсе, и отметку за него ставят сразу.
        settled.current = !toSide;
     ...
     *
     * Зовут отсюда оба, кого ждут, и трогается переезд с последнего из них: пока отметились
     * не все, звонок ничего не делает. Отметки тут же и снимаются: за один переезд место
     * меняется однажды, а звонков после него приходит сколько угодно — конец перехода
     * сообщают все, кто в этот миг ехал.
     */
    const arrive = useCallback(() => {
        if (!gone.current || !settled.current) {
            return;
        }
        gone.current = false;
    ...
    /**
     * Кадр доехал до нового размера — вместе с ним доехала и бронь под панель, и разговору
     * есть куда приезжать (см. switchLayout).
     *
     * Спрашиваем про ширину приложения: она и есть разворот. Высота сцены едет тем же
     * переходом и о своём конце сообщает отдельно, а внутри кадра едет ещё и всё, что от неё
     * считается, — до переезда это не относится.
     */
    const handleAppTransitionEnd = (event: TransitionEvent<HTMLElement>) => {
    ...
    // когда событие уже точно не придёт, и в обычной жизни не срабатывает ни разу. В развороте
    // первая половина ждёт ещё и кадр, и запас у неё свой — см. TURN_GRACE_MS.
    //
    // Отметки страховка ставит обе разом: не дождавшись одного события, ждать второго
    // бессмысленно — не будет и его.
    useEffect(() => {
        if (!moving) {
            return undefined;
        }
        const timer = window.setTimeout(
            ...
            moving === 'out' && holdUnder ? TURN_GRACE_MS : MOVE_GRACE_MS
        );
        return () => window.clearTimeout(timer);
    }, [moving, holdUnder, arrive]);
    ...
    const contentLook = [
        ...
        // Придержанный на время разворота блок вынут из раскладки: он стоит там же, где стоял,
        // и уходит вниз, а кадр за ним берёт свой конечный рост, не спрашивая у него высоты.
        holdUnder ? styles.contentAside : '',
    ...
    return (
        <div
            ...
            onTransitionEnd={handleAppTransitionEnd}
        >
                ...
                <div className={[styles.headerBar, expanded ? styles.headerBarExpanded : ''].filter(Boolean).join(' ')}>
                    ...
                    <div className={styles.headerActions}>
                        ...
                        {expanded && sideFits && (
                            <IconButton
                                onClick={() => {
                                    // Кадр в этом переезде не двигается: ждать блоку некого,
                                    // кроме себя самого (см. arrive).
                                    gone.current = false;
                                    settled.current = true;
                                    setMoving('out');
                                }}
            ...
            <Shade open={Boolean(shownMember)} onClose={() => setShownId(null)} label="Корабль" onScene={atSide}>
                {shownCard && (
                    <ShipCard
