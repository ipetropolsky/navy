 ...
 *
 * Файл общий (`shared/`): расстановку читают и браузер, рисуя овалы и разводя тесную пару
 * в кадре, и сервер, назначая место по-настоящему, — двух копий этой арифметики быть не должно.
 *
 ...
 *
 * Это тот же `Afloat`, которым описывает корабль и ход: корабль на воде один и тот же,
 * кто бы на него ни смотрел. Имя своё, потому что здесь речь о стоящих, а не об идущих.
 */
export type Standing = Afloat;
 ...
 *
 * Поле одно и то же у обеих кромок и на любом экране: рейд от окна не зависит вовсе.
 * Где эти кромки окажутся на экране, решает проекция уже в стилях (см. `--raid-reach`
 * в SeaScene.module.less): передний край рейда она выводит за кадр, дальний — поджимает внутрь.
 */
 ...
 * в поле по краям нельзя: рейд кончается там, где начинается поле (см. EDGE_MARGIN). Дальше
 * кромки и не понадобится — пару, которой не разойтись в рейде, расстановка на линию не пустит
 * (см. fitsAlongside).
 *
...
/**
 * Ближайшее свободное место к тому, что заняли перед носом. Формула одна на оба случая,
 * из-за которых сюда попадают: место увели за то время, что человек заполнял форму, или
 * переоснащение больше не помещается там, где корабль стоял, — и там и там правильный ответ
 * один: не через весь рейд, а на шаг в сторону от того, что было выбрано.
 *
 * Расстояние — слоты вглубь плюс коридоры вбок: CORRIDOR_COST слотов за один коридор.
 * Тройка — это «свой коридор дороже соседнего ровно настолько»: на три слота вглубь рейда
 * корабль отойдёт охотнее, чем на один коридор вбок, а на четыре — уже нет.
 *
 * Прогоняется по всем свободным местам (freeBerths), а не по соседним слотам того же
 * коридора, — запреты рейда (интервал, остров, размер) от этого не нарушаются, а свободное
 * место в соседнем коридоре иногда ближе, чем через два слота в своём.
 *
 * Свободных мест нет — null: тот, кто звал эту функцию, встретил занятое место потому,
 * что рейд ещё не полон, а раз так, свободное где-то да есть, но проверка дешевле,
 * чем разбирательство, если однажды это перестанет быть правдой.
 */
export const nearestBerth = (kind: ShipKind, taken: Standing[], from: Berth): Berth | null => {
    const free = freeBerths(kind, taken);
    if (free.length === 0) {
        return null;
    }
    const fromIndex = CORRIDORS.indexOf(from.corridor);
    const distance = (berth: Berth): number =>
        Math.abs(berth.slot - from.slot) + CORRIDOR_COST * Math.abs(CORRIDORS.indexOf(berth.corridor) - fromIndex);
    // Перемешиваем до сортировки: сортировка устойчива, и одинаково близкие места сохранят
    // случайный порядок — тот же приём, что и в suggestBerth.
    return shuffled(free).sort((a, b) => distance(a) - distance(b))[0];
};

/**
 * Куда в итоге встать: пожелания нет — решает вся расстановка (suggestBerth), пожелание
 * свободно — оно и есть ответ, занято — ближайшее к нему (nearestBerth). Вынесено отдельно
 * от placeShip, потому что это чистый выбор места, а placeShip следом ещё назначает сторону
 * захода и курс — разные заботы, и смешивать их в одной функции незачем.
 */
const chooseBerth = (kind: ShipKind, taken: Standing[], wanted?: Berth): Berth | null => {
    if (!wanted) {
        return suggestBerth(kind, taken);
    }
    return isBerthFree(wanted, kind, taken) ? wanted : nearestBerth(kind, taken, wanted);
};

/**
 * Ставит корабль на рейде. Место человек выбирает сам, но полагаться на этот выбор нельзя:
 * пока он раздумывал, туда мог встать кто-то другой, — и тогда корабль встаёт на ближайшее
 * к выбранному, а не куда придётся: целился он в точку на рейде, а не «куда-нибудь».
 *
...
export const placeShip = (kind: ShipKind, taken: Standing[], wanted?: Berth, course?: Side): ShipPlacement | null => {
    const berth = chooseBerth(kind, taken, wanted);
