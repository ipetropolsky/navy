...
export default defineConfig({
    testDir: './tests',
    // Каждая проверка живёт в своём хранилище, но сцена анимирована и ждать в ней приходится
    // секундами: параллелить некуда, а вот таймаут нужен щедрый. Мерка не с потолка: самый
    // длинный ход в кадре — уход с ближней линии через весь рейд — идёт около 53 с
    // (`MIN_SAIL_PACE` в shipMotion), и проверка, которая его дожидается, должна успеть
    // ещё и открыть канал, и поставить корабль в строй.
    timeout: 120_000,
