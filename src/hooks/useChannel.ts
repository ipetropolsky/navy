...
export function useChannel(
    ...
    useEffect(() => {
        ...
        // !== myIdRef.current выше и так читает ссылку, не само состояние): встал в строй
        // или вышел — подписка пересобирается заново, а не тянет прежнюю. Пока вошедший ещё
        // не участник, правило пускает его к той же обрезанной подписке, что и гостя без
        // входа (members/лента отказывают permission-denied и гаснут молча, не роняя связь —
        // см. firebaseBackend.ts, комментарий у подписки на участников); свежая же подписка,
        // заведённая после join(), уже подписана как участник. Донести участников и ленту
        // ей всё равно нечем — свой первый снимок она проглатывает точно так же (см. коммент
        // у applyEvent выше) — тем это и берёт на себя отдельный запрос в join() ниже,
        // а эта пересборка нужна лишь затем, чтобы дальнейшие чужие события подписка вообще
        // начала слышать.
    ...
    const join = useCallback(
        async (draft: MemberDraft) => {
            ...
            // Довыгружаем полный снимок в фоне, не задерживая сам join(): вошедший, ещё
            // не участник, до этой минуты видел урезанный превью-снимок (participants —
            // бортовые номера, messages: [] — см. readChannelForUser в firebaseBackend.ts),
            // и withMember выше просто дописал в этот же урезанный снимок свежепринятого
            // участника. Подписка на участников и ленту сама не донесёт настоящие данные:
            // myId сменился, эффект ниже пересоберёт её заново, а первый снимок свежей
            // подписки — это состояние, а не событие, и молча проглатывается (см. комментарий
            // в firebaseBackend.ts, subscribe). Без этого запроса вошедший так и остался бы
            // смотреть на бортовые номера и пустую ленту до первого чужого события.
            //
            // Отказ здесь не должен уронить join() — сам вход уже состоялся (сервер его принял,
            // withMember и setMyId выше это подтверждают), и терять его из-за того, что не
            // удалось перечитать канал, нельзя. Не вышло — не страшно: рейд остаётся таким,
            // каким его сейчас показывает снимок, а следующее событие подписки при случае
            // донесёт то же самое.
            void backend
                .getChannel({ channelId, userId: member.memberId })
                .then((snapshot) => {
                    setChannel((current) =>
                        current?.channel.channelId === channelId ? (snapshot ?? current) : current
                    );
                })
                .catch(() => {});
        },
