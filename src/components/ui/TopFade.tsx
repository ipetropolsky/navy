 ...
 * и на смену размеров: обход разметки в обработчике, который зовут по разу на кадр, — самое
 * верное место, чтобы лента начала запинаться.
 *
 * Размеры смотреть приходится потому, что мотаться блок начинает и без смены содержимого:
 * разговор в просторной раскладке помещается целиком, а в сжатой — уже нет, и переключение
 * раскладок ведёт его через все промежуточные высоты. Пока размеры не смотрели, полоска
 * в этом случае не появлялась вовсе: искали, когда мотать ещё было нечего, и запоминали, что
 * прокручиваемого внутри нет. Обход при этом идёт не на каждый кадр перехода: пока найденный
 * блок мотается, его и держим, — а искать заново приходится ровно тогда, когда внутри ничего
 * не мотается, то есть содержимого там немного.
 */
export default function TopFade({ className, children }: TopFadeProps) {
    ...
    useEffect(() => {
        ...
        const observer = new ResizeObserver(() => {
            ...
            if (!found || !found.isConnected || !scrolls(found)) {
                scrollerRef.current = topScroller(body);
            }
            setFade(fadeFor(scrollerRef.current));
        });
