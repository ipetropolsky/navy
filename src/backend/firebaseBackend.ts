        ...
        subscribe: ({ channelId, userId, onEvent }) => {
            ...
            const unsubscribeMembers = onSnapshot(
                ...
                (failure) => {
                    ...
                    // Отказ подписке, которая только что работала, — это и есть весть
                    // о высадке, другой не будет. Правило пускает к участникам, пока сам стоишь
                    // на рейде; сняли с рейда — перестало пускать, и Firestore отвечает на это
                    // не удалением записи в docChanges, а отказом всему запросу разом. Того
                    // самого `removed`, которым уходят из кадра прочие, про себя не дождёшься
                    // никогда: вкладка так и осталась бы стоять на рейде, с которого её ссадили, —
                    // с полным списком соседей и со своим кораблём в кадре (#79). Потому и
                    // рождаем уход сами, от своего имени: участие адресуется личностью напрямую
                    // (memberId === userId, см. join в functions/src/raid.ts), так что своё имя
                    // в этой подписке известно — оно и есть userId.
                    //
                    // «Которая только что работала» — вот на что смотрит firstMembers, и смотрит
                    // ровно наоборот, чем в самой подписке выше. Там поднятый флаг значит
                    // «первый снимок ещё не проглочен», здесь — «снимка не было вовсе, а значит,
                    // и участником эта вкладка не была»: гостю у ворот тот же самый отказ
                    // приходит сразу же, первым делом, и высадкой он, разумеется, не является.
                    //
                    // userId не передан вовсе — некого и назвать ушедшим: звавшему не важен
                    // сам факт входа (см. subscribe в types.ts), своего корабля у него нет.
                    if (!firstMembers && userId) {
                        onEvent({
                            eventId: randomEventId(),
                            channelId,
                            at: Date.now(),
