...
test.describe('телефон', () => {
    ...
    test('кнопки берут всю ширину: в строку, пока подписи влезают, и столбиком, когда нет', async ({ page }) => {
        // Форма настройки корабля: в ней кнопок две — «Готово» и «Отставить», — и делить между
        ...
        // телефона: «Готово» с «Отставить» коротки и на 320px стоят в строку свободно, — но
...
/**
 * Выходов из шторки четыре: крестик в верхнем углу, нажатие мимо, свайп вниз за ручку и Esc.
 * Ручка — единственное место, за которое шторку тянут: внутри неё нажимают кнопки, мотают
 * содержимое и выделяют текст, и отбирать у них движение пальца нечем. Esc проверяется отдельно,
 * в channel.spec.ts — там же, где и остальные слои, которые он умеет закрывать.
 *
 ...
 * не свайпом, а крестиком или словом рядом с ним в полосе кнопок.
 *
 ...
 * ни броском. И форма от свайпа не закрывается — закрывает её «Отставить», а под ней остаётся тот
...
test('коробку с формой тянут за ручку в обе стороны, а до пола её не сминают', async ({ page }) => {
    ...
    // вовсе: закрывают её крестиком и «Отставить».
    ...
    // «Отставить» снимает верхний слой, и под ним остаётся нижний — тот самый список, из которого
