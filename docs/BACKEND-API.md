...
// тогда уже негде. Снимок кладёт тот, кто пишет сообщение; у ссылки на старшего и у события
// печати его нет — они говорят о том, кто на рейде сейчас.
interface MemberRef {
    memberId: string;
    look?: AuthorLook;
}
...
и событие `typing` исчезают из контракта совсем. Разбор — в
[задаче №6](https://github.com/ipetropolsky/navy/issues/6).
