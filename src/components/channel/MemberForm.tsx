...
export default function MemberForm({
    ...
    // Esc — то же самое, что и «Отставить», и только при переоснащении: у формы входа своя
    // «Отмена» тоже есть, но она сворачивает форму до закрытого вида, а не возвращает на рейд —
    // возвращаться там пока некуда, человек ещё гость, и Esc туда падать не должен вовсе
    // (см. `atGate` в App.tsx).
    useEffect(() => {
    ...
    // А вот отказ подписан по-разному. При переоснащении у формы есть куда вернуться — на рейд,
    // где корабль уже стоит, — и «Отставить» здесь то же самое слово, каким закрывают её
    // крестиком и Esc (см. `onClose` у Panel и обработчик ниже). У входа возврата нет: «Отмена»
    // только сворачивает форму до закрытого вида, набранное остаётся, и корабля на рейде ещё нет.
    const cancelLabel = mode === 'edit' ? 'Отставить' : 'Отмена';
    ...
    return (
        <Panel
            ...
            // Крестик — только при переоснащении, тем же выходом, что и «Отставить» рядом.
            // У входа он не встаёт: там нет возврата на рейд, до которого пока не дошли
            // (см. cancelLabel), и второй кнопки-выхода вдобавок к «Отмене» там не нужно.
            onClose={mode === 'edit' ? onCancel : undefined}
            actions={
                <>
                    {onCancel && (
