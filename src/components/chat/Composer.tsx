 ...
 *
 * Каждая буква — событие бэкенда, а оно доходит до всех вкладок и перерисовывает в каждой
 * всё приложение: лампа Морзе живёт в кадре. При беглом наборе это десяток полных отрисовок
 * в секунду на пустом месте.
 *
 * Копится набранное куском и уходит раз в треть секунды. На глаз ничего не меняется: лампа
 * и так проигрывает очередь посегментно и своей скоростью (см. useMorseLamp), а до неё буквы
 * идут по сети — доля секунды там теряется и без нас.
 *
