 ...
 *
 * Ключ — `kilvater.outbox.<userId>.<channelId>`: `userId`, а не `memberId`, хотя на обоих
 * бэкендах они сегодня равны, — потому что ящик про то, кто набирал текст, а не про то,
 * каким кораблём это делалось в этом канале. `userId`, а не `memberId`, и по имени понятно,
 * какая из двух ролей тут важна.
 *
