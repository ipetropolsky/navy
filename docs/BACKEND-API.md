...
запись и рассылает её обычным `message-added`. У таких записей стоит `kind: 'system'`, начинаются
они с того, как корабль зовут целиком, а изменившееся помечено звёздочками — интерфейс делает
из пометки жирное:
