...
function SeaScene({
    ...
    const finishMotion = (id: string): void => {
        ...
        if (!relocatingIds.current.has(id)) {
            replayLeft.current.delete(id);
            replaySeek.current.delete(id);
            // Доигранный уход — это весь манёвр, а не половина: корабль по-настоящему покинул
            // рейд, а не отошёл на перегон внутри него. Не забудь его тут — и вернувшийся тем же
            // id (тот же участник, уйти с рейда и тут же встать заново) застанет старую метку
            // места (shownById) и старую метку «уже видели» (seenIds): сравнение примет его
            // не за нового, заходящего с кромки, а за перезаходящего со старого места, — и вместо
            // одного чистого входа получится невидимый лишний уход-и-пауза перед настоящим (issue
            // про рывок после «Уйти с рейда» → сразу «Встать на рейд», см. `manoeuvreGuard.ts`).
            shownById.current.delete(id);
