...
export default function App() {
    ...
    // вверх притягивает обратно. Обратные движения кадр не трогает, и свайп страницы
    ...
    const taken = Math.max((formOpen || listOpen ? boxSize : size) - lowered, 0);
     ...
     * указателя выносит палец за его кромку. Записываем начало свайпа, а не считаем сдвиг
    ...
    return (
        <div
            ...
            //   для свайпа — он держится видимой кромки.
        ...
        >
            ...
            >
                {/* Ручка для хвата. Тянет она то же самое и тем же способом, что и коридор
                    над ней (handleGripDown): у коробки одна кромка, и двигают её одним
                    движением. Нажатие до коробки не доходит — иначе форма постановки
                    в строй, которую приспускают свайпом по любому месту, поехала бы
                    вместе с размером разговора. */}
                {!atSide && (
                    <div
            ...
            {formSlide.mounted && me && (
                <div
                    ...
                    // Приспускают её тем же свайпом, что и форму постановки в строй: коробка
                    // внизу экрана у них одна, и правила движения одни. Сбоку свайпа нет —
                    // панель двигают за полоску на её кромке.
                    {...(atSide ? {} : lower.handlers)}
                    onTransitionEnd={formSlide.onTransitionEnd}
                >
                    {/* Ручка у формы — та же рисочка, но своего обработчика у неё нет:
                        форму тянут за любое место (lower.handlers на самой коробке),
                        и ручке остаётся только показать, что тянуть можно и куда. */}
                    {!atSide && (
