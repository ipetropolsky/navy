 ...
 * отпустят.
 *
 * Отсюда двигаются модальные шторки — карточка чужого корабля и прощание с рейдом. Коробка
 * разговора считает свой свайп сама (`gripHandlers` в App): у неё две оси, память о размере
 * и клавиатура, и общего с этим хуком у неё пока только `trackFling` и `settleMagnet`.
 * Расходиться этим двоим нельзя ничем сверх настроек: один и тот же бросок пальца обязан
 * одинаково кончаться и там, и там. Что уже разошлось и как сводить — docs/SHEETS.md.
 *
