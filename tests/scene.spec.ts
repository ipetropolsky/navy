...
/**
 * Ход, начавшийся без этой вкладки, она доигрывает с середины (issue #78).
 *
 * Манёвр в кадре — не записанный факт, а разница: вкладка рисует корабль там, где нарисовала
 * его в прошлый раз, сравнивает с тем, что пришло с «сервера», и разницу отыгрывает ходом.
 * У вкладки, открывшейся посреди манёвра, прошлого раза нет — и раньше она заставала корабль
 * уже на новом месте, будто он туда прыгнул. Теперь корабль везёт с собой запись хода
 * (`Member.manoeuvre`): откуда пошёл, когда тронулся и насколько хода отмерено, — и свежая
 * вкладка притворяется, что сама нарисовала его там, откуда он пошёл. Дальше разницу
 * подхватывает та же машинка, что и всегда.
 *
 * Сколько хода уже прошло, уходит в стили отрицательной задержкой: переход начинается сразу,
 * но с середины. Её здесь и спрашиваем — она и есть признак того, что ход подхвачен, а не начат
 * заново. Само место на рейде проверяют юниты (`shipMotion.test.ts` и `localBackend.test.ts`);
 * браузеру достаётся то, чего в счёте нет.
 *
 * Проверяется перезагрузка, а не вторая вкладка, и не по бедности: посторонней вкладке рейда
 * не видно вовсе — не участнику канала бэкенд отдаёт превью без единого корабля (см.
 * `needsPreview` в localBackend, а на настоящем сервере — `isMember` в firestore.rules).
 * Значит, застать чужой манёвр может только тот, кто на этом рейде уже стоит, — и первый
 * такой случай как раз перезагрузка. Код у обоих один и тот же: вкладка после перезагрузки
 * ничем не отличается от вкладки, открытой впервые, — своей памяти о кадре у неё нет.
 *
 * Время перезагруженной вкладке возвращается обычное, не ускоренное. Свой срок хода она
 * считает сама, своей меркой (см. `manoeuvreSeconds`), и в неускоренном времени тот срок
 * вдесятеро длиннее прошедшего — корабль остаётся в пути надолго, и застать его в пути можно
 * без гонки с часами. Эта самая разница в сроках и есть то, ради чего перемотка считается
 * остатком, а не показаниями чужих часов: у каждой вкладки свой кадр и свой ход.
 */
test('перезагрузка посреди хода застаёт корабль в пути', async ({ page }) => {
    takes(20);
    // Рейд свой и корабль на нём один: в демо-канале расстановка каждый раз новая, и что
    // застанет перезагрузка — как повезёт.
    await openNewChannel(page, 'dogonka');
    await page.getByText('Пограничный сторожевой катер', { exact: true }).click();
    await page.locator('[data-berth="8-center"]').click();
    await join(page, 'Стриж', '111');
    await myShipParked(page);

    // Уходит он через весь рейд — с ближней линии на дальнюю: такая перемена места
    // отыгрывается полным манёвром в три колена (уход, пауза, заход), а не переходом по воде.
    await openShipForm(page);
    await page.locator('[data-berth="1-center"]').click();
    const started = Date.now();
    await page.getByRole('button', { name: 'Готово' }).click();
    await expect(page.locator('[data-motion]'), 'корабль не тронулся с места').toHaveCount(1);
    ...
    const lane = page.locator('[data-motion]');
    await expect(lane, 'после перезагрузки корабль оказался сразу на новом месте').toHaveCount(1);
    // Доигрывается манёвр с начала, а не с конца: корабль сперва уходит со старого места
    ...
    // Отматывать вперёд больше, чем ход и правда идёт, нельзя: перемотка — прошедшее время,
    // а не запас. Сверху — то же прошедшее время, померенное снаружи, вместе с перезагрузкой.
    expect(-delay, 'вкладка отмотала ход дальше, чем он успел пройти').toBeLessThanOrEqual(elapsed);

    // Место на рейде за корабль давно решено: манёвр — только показ того, что уже случилось.
    const state = await readState(page);
    const [moved] = Object.values(state.channels).find((one) => one.channel.slug === 'dogonka')!.members;
    expect(`${moved.place.slot}-${moved.place.corridor}`, 'корабль встал не на выбранное место').toBe('1-center');
});
