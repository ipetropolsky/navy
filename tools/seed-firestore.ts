...
// Время — от полуночи, а не число из прошлого: демо не должно выглядеть перепиской недельной
// давности. В пределах одного запуска (и двух подряд — при проверке идемпотентности) полночь
// одна и та же, так что документы выходят одинаковыми побайтово.
//
// Полночь эта — вчерашняя, если утро, которым идёт разговор, сегодня ещё не наступило. Иначе
// демо оказывается в будущем, а это не мелочь оформления: лента выстроена по времени, и всё,
// что в канале напишут после, встанет в середину разговора, — а счётчик непрочитанного,
// который считает от метки последнего виденного сообщения, промолчит вовсе. Тот же выбор
// и по той же причине делает `minutesAfterMidnight` в src/backend/seed.ts.
const ENDS_AT = { hours: 9, minutes: 12 };
const midnight = new Date();
midnight.setHours(0, 0, 0, 0);
const ends = new Date(midnight);
ends.setHours(ENDS_AT.hours, ENDS_AT.minutes, 0, 0);
if (ends.getTime() > Date.now()) {
    // Календарём, а не вычитанием суток: в ночь перевода часов сутки не двадцать четыре часа.
