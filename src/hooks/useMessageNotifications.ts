...
export function useMessageNotifications(channel: ChannelSnapshot | null, myId: string | null, watching: boolean): void {
    ...
    useEffect(() => {
        ...
        // случае: решение может ещё прийти (см. ниже), а до тех пор реплика не должна
        // потеряться незамеченной.
        unseenCount.current += 1;
        startFlashing();
        if (Notification.permission === 'default') {
            // Спрашиваем не раньше первой реплики, на которую было бы что показать: человек,
            // который просто открыл приложение и ни разу не выходил из вкладки, вопроса
            // не увидит вовсе — разрешение просить не о чем.
