...
export default function App() {
    ...
    const picking = !loading && Boolean(channel) && (editing || (!me && joining));
    ...
    const baseContent = (
        <>
            ...
            {/* Форма постановки в строй — это и есть содержимое блока: разговора у того, кто
                ...
                в двух видах, и набранное в ней закрытие переживает. Пока закрывается она
                только сама — потяг, которым её будут приспускать, чтобы разглядеть рейд,
                ещё впереди (docs/TODO). */}
            {!loading && channel && !me && (
                <MemberForm
                    mode="join"
                    crew={members}
                    ...
                    // Кнопки «отмена» у входа нет: закрывать форму обратно человек будет
                    // не кнопкой, а потягом — тем же, каким приспускают любую шторку, чтобы
                    // разглядеть рейд под ней. Пока этого потяга нет, набранное всё равно
                    // держится за формой: она остаётся на месте, закрытая и открытая.
                />
            )}
            {channel && me && (
                <>
