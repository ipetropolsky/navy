 ...
 * Высота сложенной при этом одна на обе раскладки (SHADE_PEEK_HEIGHT): с неё и там, и там
 * читаются три последние реплики с полем ввода.
 */
...
/**
 * Сколько накрутить колесом, чтобы шторка шагнула на ступень, px прокрутки. Один щелчок колеса
 * в браузере — около сотни, то есть ступень идёт за щелчок с небольшим.
 *
 * Лежит здесь, а не в самой шторке, по той же причине, что и остальная арифметика: это число,
 * которое проверяют числом. Проверкам его иначе и не достать — они не собирают стилей.
 */
export const WHEEL_STEP = 120;

/**
 * Соседняя ступень в заданную сторону: вверх (1) или вниз (−1). На краю лестницы возвращается
 * та же самая — в отличие от `nextStop`, который с верхней ступени идёт по кругу в нижнюю.
 * Разница по делу: ручка — один орган управления, и ходить ей приходится туда-обратно, а у
 * колеса стороны две, и заворачивать его с верхней ступени в щёлку значило бы отвечать
 * движением вниз на движение вверх.
 */
export const stepStop = (stop: ShadeStop, mobile: boolean, direction: 1 | -1): ShadeStop => {
    const stops = shadeStops(mobile);
    // Ступени, которой в этой лестнице нет, соответствует нижняя: так на десктопе ведёт себя
    // половина, оставшаяся с телефонной раскладки после того, как окно растянули.
    const from = Math.max(stops.indexOf(stop), 0);
    return stops[Math.min(Math.max(from + direction, 0), stops.length - 1)];
};
