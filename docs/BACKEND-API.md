...
Место выбирает человек — в форме корабля, овалом на воде, — но приходит оно тем же путём,
что и всё остальное: полем `berth` в черновике участника.

```ts
await backend.join({ channelId, member: { ...draft, berth: { slot: 7, corridor: 'right', left: 78.1 } } });
...
форме нужно нарисовать овалы до того, как что-то отправлено, а считаются они из того же
списка кораблей, который у вкладки и так есть.
