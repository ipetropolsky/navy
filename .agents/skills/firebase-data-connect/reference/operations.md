...
mutation DeleteOldDrafts @auth(level: USER) {
    post_deleteMany(where: { status: { eq: "draft" }, createdAt: { lt_time: { now: true, sub: { days: 30 } } } })
}
```
...
```graphql
mutation CreateTodoWithItem($listName: String!, $itemText: String!) @auth(level: USER) @transaction {
