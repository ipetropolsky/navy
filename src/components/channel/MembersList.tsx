 ...
 *
 * Сам по себе список — только колонка со своей прокруткой: показывают его двое — шторка
 * поверх обычного вида (`MembersSheet`) и шторка полноэкранного (`Shade`), — и рамка,
 * затемнение и выезд у них свои.
 */
export default function MembersList({ members, myId, seniorId, onEditMe, onLeave, onKick, onHail }: MembersListProps) {
    ...
    return (
        <div className={styles.list}>
            ...
            {myId && (
                // Список бывает длиннее того, что его показывает, и кнопки уезжают под обрез
                // так же, как в форме корабля, — значит и держим мы их так же.
                <Actions wide pinned>
