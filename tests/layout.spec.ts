...
/**
 * Лента возвращается первым кадром дороги, а не в конце её.
 *
 * Порог `FEED_MIN` считается по тому размеру, к которому коробка едет, а не по тому, какая
 * она сейчас: брошенный от пола разговор знает про свою точку сразу, а ехать к ней будет
 * полсекунды (`--chat-move`). Держать всё это время пустую коробку не за чем — место под ленту
 * в ней появляется с первого же кадра, — а человек эти полсекунды считает не движением,
 * а задержкой ответа на своё движение.
 *
 * Смотрим покадрово: движение останавливают на его начале и там же меряют. Иначе видно было бы
 * только приехавшее — а приехавшее выглядит одинаково и когда лента ждала конца дороги,
 * и когда не ждала.
 */
test('лента возвращается в начале дороги, а не в конце', async ({ page }) => {
    // Время обычное: проверка смотрит на первый кадр дороги, а ускоренная вдесятеро она короче,
    // чем один заход в страницу снаружи, — останавливать было бы уже нечего.
    await unhasten(page);
    await page.setViewportSize(PHONE);
    await openChannel(page, DEMO, ALBATROS);
