 ...
 * никто не станет. Взято примерно втрое больше — так манёвр укладывается в те секунды,
 * что на него смотрят, и при этом остаётся неспешным.
 *
 * Это средняя по манёвру. В самый быстрый момент корабль идёт втрое быстрее: он разгоняется
 * и тормозит, а кривая в стилях распределяет скорость внутри прогона (замер по самим кривым:
 * пик выше среднего в 2.86 раза у захода и ухода, в 3.01 у перестановки).
 */
const ROADSTEAD_KNOTS_LONGEST = 10;
const ROADSTEAD_KNOTS_SHORTEST = 16;
