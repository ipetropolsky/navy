...
export default function App() {
    ...
    /**
     * Ушёл ли только что сам — или его сняли с рейда, старшим или из другой вкладки. Различать
     * приходится вручную, флагом: событие участников (`member-left`, см. useChannel.ts) при
     * высадке и при уходе выглядит одинаково, участник просто пропадает из списка, — сервер
     * причины не называет, да и называть в этот момент уже некому: снятую вкладку никто
     * не спрашивает. Ставим флаг до самого ухода, а не в его ответе: `myId` внутри useChannel
     * обнуляется синхронно с resolve `leave()` (см. её комментарий там), и без этого запаса
     * разница между «ушёл сам» и «сняли без спроса» до эффекта ниже попросту не доедет.
     */
     ...
     * выше). Высадка — решение старшего, а не своё, и пускать по старой памяти вкладки в обход
     * него нельзя (#80): не по адресу самого канала эту память стирать, как в эффекте прихода
     * выше, — тут и вкладку никто не покидал, — а по тому, что место на рейде исчезло не от
     * своего решения. Отличить одно от другого можно только `leftByMe`, для App оба случая
     * выглядят одинаково — `me` меняется на null.
     */
    useEffect(() => {
        if (!me) {
            if (!leftByMe.current) {
     ...
     * ещё раз напрямую.
     *
     * Проходит это стороной чужого гостя, а не только заведшего канал, только если `joining`
     * успел стать true раньше, чем встал этот же экран, — а это ему неоткуда взять: единственные
     * два места, что включают `joining` (`open-join` — см. `handleCheckCode` ниже и `onOpen`
     * у формы), сами лежат за тем же `needsCode` в разметке ниже и потому не нажимаются, пока
     * код не подтверждён. Высаженного (#80) это тоже не касается: высадить старший может только
     * не старшего, а `joining` от `arrive` достаётся исключительно тому, кто канал завёл сам, —
     * то есть будущему старшему, которого не высадит никто.
     */
     ...
     *
     * На главной этой кнопки нет вовсе — она и так дом. У формы создания канала свой выход,
     * крестиком и «Отставить» в её собственной полосе кнопок (см. CreateChannel.tsx):
     * оба стоят рядом с тем, что закрывают, а не разнесены на разные концы экрана.
     *
    ...
    const goBack = atEntry ? route.openHome : null;
    ...
    //
    // Поверх списка бывает либо форма своего корабля (закрывает её тогда «Отставить» в MemberForm,
    ...
    const handleLeaveConfirm = (course: string, nextOwnerId?: string) => {
        ...
        void channelState
            ...
            // до настоящей будущей высадки и не скрыл её от эффекта у `verifiedCode`.
            .catch((failure: unknown) => {
                leftByMe.current = false;
        ...
        <>
            ...
            {!waiting && route.channel && !channel && !loadError && (
                <Panel
                    ...
                    actions={<Button onClick={route.openHome}>Полный назад</Button>}
                />
            )}
